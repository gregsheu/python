...
def get_snap(ip, channel, eventstart, user, password, *args):
    ...
    newfilename = '%s/%s' % (date_dir, filename)
    webviewlink = gdrive(drive_service, third_parent_id, newfilename)
    gmail(gmail_service, webviewlink)
    return '%s/%s' % (date_dir, filename)
    #return '%s' % filename

def get_video(ip, channel, eventstart, eventend, *args):
    gmail_service = args[0]
    drive_service = args[1]
    third_parent_id = args[2]
    newtime = eventstart.replace(' ', '')
    ...
    (
        ffmpeg
        .output(r, '/tmp/%s/tripvideo-%s-%s.mp4' % (date_dir, str(channel)+ip.split('.')[-1], newtime), ss=0, t=10, r=5, vb='512k', vcodec='libx264', maxrate='512k', bufsize='1024k', g=2, pix_fmt='yuvj420p')
    ...
    with requests.get(url, auth=HTTPDigestAuth(user, password), stream=True) as resp:
        ...
        for line in resp.iter_lines():
            ...
            if 'Appear' in decoded_line:
                ...
                # 2021-03-17 move goolge drive and gmail to snap and video
                t3 = threading.Thread(target=get_snap, args=(cam_ip, 1, eventstart, user, password, *args,))
                t3.daemon = False
                t3.start()
                # 2021-03-17 move goolge drive and gmail to snap and video
                t4 = threading.Thread(target=get_video, args=(cam_ip, 1, eventstart, eventend, *args,))
                t4.daemon = False
                t4.start()
                time.sleep(0.1)
                alarm_sound(alarm)
