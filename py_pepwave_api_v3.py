...
def send_udp(msg):
    ...
    telegraf_socket = "/var/task/telegraf.sock"
...
def get_client(url, sessionid):
    ...
    for i in response_json.values():
        client_list += i
    send_udp(json.dumps(client_list).encode('utf-8'))
...
def get_gps(url, sessionid):
    ...
    response = requests.get(endpoint, headers=header)
    response_json = response.json()['response']
    send_udp(json.dumps(response_json['location']).encode('utf-8'))
...
def get_signal(url, sessionid):
    ...
    for k, v in response_json.items():
        if k == '2':
            send_udp(json.dumps(v['band'][0]['signal']).encode('utf-8'))
...
def main(argv):
    ...
    while True:
        #get_session_geoip()
        #time.sleep(1)
        get_client(url, sessionid)
        time.sleep(1)
        get_gps(url, sessionid)
        time.sleep(1)
        get_signal(url, sessionid)
        time.sleep(1)
        get_temperature()
        time.sleep(3)
