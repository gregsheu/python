...
def file_structures(drive_service):
    ...
    top_parent_folder = 'KingSolarman'
    top_parent_id = None
...
def gdrive(drive_service, third_parent_id, filename):
    ...
    logme('Upload file %s <br>' % filename)
    share_file_link(drive_service, file_id)
    webviewlink = get_file_link(drive_service, file_id)
    logme('File shared <a href="%s">%s</a> <br>' % (webviewlink, webviewlink))
    return webviewlink
...
def setlogger():
    ...
    handler = RotatingFileHandler('/tmp/carplate.html', maxBytes=2048000, backupCount=5)
...
def events(cam_ip):
    ...
    #snap_url = 'http://%s/cgi-bin/snapManager.cgi?action=attachFileProc&Flags[0]=Event&Events=[All]&heartbeat=5' % cam_ip
    user = 'admin'
    password = 'admin12345'
    channel = 1
    ...
    with requests.get(snap_url, params=param, auth=HTTPDigestAuth(user, password), stream=True) as resp:
        bytes = b'' 
        #print('third_parent_id %s' % third_parent_id)
        for chunk in resp.iter_content(chunk_size=1024):
            ...
            #Checking if a day is passed, so create a new date folder
            if third_parent_folder != date_dir:
                third_parent_id, third_parent_folder = file_structures(drive_service)
                print('third_parent_id 2 %s' % third_parent_id)
            ...
            if finda != -1 and findb != -1:
                ...
                if findc != -1:
                    ...
                    if plate == '':
                        plate = 'No Plate'
                    logme('Detected car plate number <b> %s </b><br>' % plate)
...
def main():
    ...
    url = "http://:8080/api"
    url = "https://166.193.35.44:8443/api"
    sessionid = login(url)
