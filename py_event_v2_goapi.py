...
def upload_file(drive_service, filename):
    ...
    media = MediaFileUpload('/tmp/%s' % filename, mimetype='video/mp4', chunksize=1024*1024, resumable=True)
    #media = MediaFileUpload('/tmp/%s' % filename, mimetype='video/mp4')
    print(media)
...
def drive_scopes():
    ...
    if os.path.exists('kingsolarman88.pickle'):
        with open('kingsolarman88.pickle', 'rb') as token:
            creds = pickle.load(token)
    # If there are no (valid) credentials available, let the user log in.
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('kingsolarman88.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open('kingsolarman88.pickle', 'wb') as token:
            pickle.dump(creds, token)
    ...
    with requests.get(url, auth=HTTPDigestAuth(user, password), stream=True) as resp:
        ...
        for line in resp.iter_lines():
            ...
            if 'Appear' in decoded_line:
                t = threading.Thread(target=alarm_sound, args=(alarm,))
                t.daemon = False
                t.start()
                cur_time = time.time()
...
def main():
    #cam_ip = os.getenv('CAM')
    #events(cam_ip)
    socket.setdefaulttimeout(3600)
    drive_service = drive_scopes()
    filename = 'outpy.mp4'
    upload_file(drive_service, filename)
