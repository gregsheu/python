...
def gmail_scopes():
    ...
    # time.
    if os.path.exists('/home/ubuntu/kingsolarman77.pickle'):
        with open('/home/ubuntu/kingsolarman77.pickle', 'rb') as token:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('/home/ubuntu/kingsolarman77.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open('/home/ubuntu/kingsolarman77.pickle', 'wb') as token:
    ...
    with requests.get(url, auth=HTTPDigestAuth(user, password), stream=True) as resp:
        ...
        for line in resp.iter_lines():
            ...
            if 'Appear' in decoded_line:
                ...
                eventend = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(cur_time+7))
                ...
                if counter <= 2:
                    ...
                    filename2 = get_snap(cam2_ip, 1, eventstart, user, password)
                    webviewlink2 = gdrive(drive_service, third_parent_id, filename2)
                    filename3 = get_snap(cam3_ip, 1, eventstart, user, password)
                    webviewlink3 = gdrive(drive_service, third_parent_id, filename3)
                    filename4 = get_snap(cam4_ip, 1, eventstart, user, password)
                    ...
                    filename1 = convert_dav('192.168.1.109', 1, eventstart, eventend)
                    time.sleep(1)
                    webviewlink1 = gdrive(drive_service, third_parent_id, filename1)
                    filename2 = convert_dav('192.168.1.109', 2, eventstart, eventend)
                    time.sleep(1)
                    webviewlink2 = gdrive(drive_service, third_parent_id, filename2)
                    filename3 = convert_dav('192.168.1.109', 3, eventstart, eventend)
                    time.sleep(1)
                    webviewlink3 = gdrive(drive_service, third_parent_id, filename3)
                    filename4 = convert_dav('192.168.1.109', 4, eventstart, eventend)
                    time.sleep(1)
                    webviewlink4 = gdrive(drive_service, third_parent_id, filename4)
                    webviewlink = '%s\n%s\n%s\n%s' % (webviewlink1, webviewlink2, webviewlink3, webviewlink4)
                    gmail(gmail_service, webviewlink)
